...
# to install the last release of Rudder 'latest' automatically.
...
echo "deb http://www.rudder-project.org/apt-latest/ squeeze main non-free" >> /etc/apt/sources.list
echo "deb-src http://www.rudder-project.org/apt-latest/ squeeze main non-free" >> /etc/apt/sources.list
