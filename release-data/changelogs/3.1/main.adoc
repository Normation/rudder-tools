...
* Fixed: Tooltips don’t disappear on safari
...
documentation (https://www.rudder-project.org/redmine/issues/9800[#9800])

==== Miscellaneous

* Fixed: Typo in root of policy library tree name
...
* Fixed: Unneeded scrollbar when using ncf builder in an iframe
(https://issues.rudder.io/issues/9164[#9164])
* Fixed: Cannot save new generic method with some constraints
...
* Fixed: Techniques use $ for paths of resources files (ex:
...
* Fixed: JS engine tests broken due to security exception when using
...
* Fixed: When a node send reports with an unknown configID, no expected
reports are showned
...
* Add rudder-agent general information command
