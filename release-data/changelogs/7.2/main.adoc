...
* Fixed: resources sent to the browser are not compressed when using a custom certificate
...
* Fixed: aix postinst doesn't install a crontab for rudder agent check
...
* Fixed: when there are no python installed on a node, template from jinja2 and packages generic method fail without explaining why
...
* Fixed: Campaign json should be versioned
...
* Fixed: Cleanup cron job for relay not removed on upgrade of 7.2
...
* Add reason field to disabled field in campaigns and campaign events
...
* Fixed: Upgrading relay to 7.2 fails on ubuntu 22
...
* Fixed: technique from technique editor not committed in git
