...
* Fixed: Add -XX:+UseStringDeduplication in G1GC optional config
...
* Allow to get information from the node inventory to use them in Directives and ncf techniques
...
* Fixed: Generation wrongly warns with"non stable policy on node" when the same directive is used in two rules
...
* Fixed: Disk usage in documentation is insufficiently precise
...
* Fixed: Postgres fills up and can't be vacuumed
...
* Fixed: Alignment of radio button on Settings page is still imperfect
...
* Fixed: Imperfect alignment of radio button
...
* Fixed: Tooltip on the source of property is not displayed in Rudder
...
* Update main menu: plugin at top level, technique editor in
